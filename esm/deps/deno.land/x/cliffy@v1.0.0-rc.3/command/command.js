...
import { UnknownTypeError, ValidationError as FlagsValidationError, } from "../flags/_errors.js";
...
import { bold, brightBlue, red } from "./deps.js";
...
export class Command {
    ...
    async executeExecutable(args) {
        ...
        try {
            const cmd = new dntShim.Deno.Command(command, {
                args,
            });
            const output = await cmd.output();
            if (!output.success) {
                dntShim.Deno.exit(output.code);
            }
        }
        catch (error) {
    ...
    async findEnvVar(names) {
        for (const name of names) {
            const status = await dntShim.Deno.permissions.query({
                name: "env",
                variable: name,
            });
